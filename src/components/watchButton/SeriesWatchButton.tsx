...
const SeriesWatchButton = ({ itemID, themeColor, refetchProgression, watchHistory, name }: IWatchButtonProps) => {
  ...
  const markAsWatched = trpc.tv.markSeriesAsWatched.useMutation({
    ...
    onError: () => {
      toast(`Failed to add all episodes for ${name} to watched`, {
        icon: <IoMdInformation className="text-3xl text-blue-500" />,
      });
    },
  });

  const removeFromWatched = trpc.tv.removeSeriesFromWatched.useMutation({
    onMutate: () => {
      setState("loading");
    },
    onSuccess: () => {
...
export default SeriesWatchButton;
